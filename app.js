...
if (process.env.DEBUG) {
  ...
  statsd.send = wrap(
    statsd.send.bind(statsd),
    console.log.bind(null, "Intercepted: statsd.send(%s):")
  );
}
...
function processLine(line, prefix, defaultTags) {
  ...
  // Router metrics
    ...
    let tags = tagsToArr(
      _.pick(line, ["dyno", "method", "status", "host", "code", "desc"])
    );
    ...
    statsd.distribution(
      prefix + "heroku.router.request.connect",
      extractNumber(line.connect),
      tags
    );
    statsd.distribution(
      prefix + "heroku.router.request.service",
      extractNumber(line.service),
      tags
    );
    if (line.at === "error") {
...
function tagsToArr(tags) {
  return _.transform(
    tags,
    (arr, value, key) => arr.push(key + ":" + value),
    []
  );
}
...
function loadAllowedAppsFromEnv() {
  assert(
    process.env.ALLOWED_APPS,
    "Environment variable ALLOWED_APPS required"
  );
