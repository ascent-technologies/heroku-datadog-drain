...
describe("Heroku Datadog Drain", function () {
  beforeEach(function () {
    ...
    sinon.spy(StatsD.prototype, "distribution");
  });

  afterEach(function () {
    StatsD.prototype.histogram.restore();
    StatsD.prototype.increment.restore();
    StatsD.prototype.distribution.restore();
  });
  ...
  it("sends router response metrics and error count to statsd", function () {
    return request(app)
      ...
      .send(
        `255 <158>1 2015-04-02T11:52:34.520012+00:00 host heroku router - at=info method=POST path="/users/40/emails/7ab797c5-50a8-473c-91b3-450b81f86a05" host=myapp.com request_id=c1806361-2081-42e7-a8aa-92b6808eac8e fwd="24.76.242.18" dyno=web.1 connect=1ms service=37ms status=201 bytes=828\n
      ...
      .expect("OK")
      .then(function () {
        expect(StatsD.prototype.distribution.args).to.deep.equal([
          [
            "heroku.router.request.connect",
            1,
            [
              "dyno:web.1",
              "method:POST",
              ...
              "host:myapp.com",
              "default:tag",
              "path:/users/:id/emails/:uuid",
            ],
          ],
          [
            "heroku.router.request.service",
            37,
            [
              "dyno:web.1",
              "method:POST",
    ...
    return request(app)
      ...
      .then(function () {
        expect(StatsD.prototype.histogram.args).to.deep.equal([
          [
            "heroku.dyno.load.avg.1m",
            0.01,
            ["dyno:web.1", "default:tag"],
          ],
          [
            "heroku.dyno.load.avg.5m",
            0.02,
            ["dyno:web.1", "default:tag"],
          ],
          [
            "heroku.dyno.load.avg.15m",
            0.03,
            ["dyno:web.1", "default:tag"],
          ],
          [
            "heroku.dyno.memory.total",
            103.5,
            ["dyno:web.1", "default:tag"],
          ],
          [
            "heroku.dyno.memory.rss",
            94.7,
            ["dyno:web.1", "default:tag"],
          ],
          [
            "heroku.dyno.memory.cache",
            0.32,
            ["dyno:web.1", "default:tag"],
          ],
          [
            "heroku.dyno.memory.swap",
            8.48,
            ["dyno:web.1", "default:tag"],
          ],
          [
            "heroku.dyno.memory.pgpgin",
            36091,
            ["dyno:web.1", "default:tag"],
          ],
          [
            "heroku.dyno.memory.pgpgout",
            11765,
            ["dyno:web.1", "default:tag"],
          ],
        ]);
  ...
  it("sends PostgreSQL metrics to statsd", function () {
    return request(app)
      ...
      .then(function () {
        expect(StatsD.prototype.histogram.args).to.deep.equal([
          [
            "heroku.postgres.current_transaction",
            6709,
            ["source:HEROKU_POSTGRESQL_TEAL", "default:tag"],
          ],
          [
            "heroku.postgres.db_size",
            18032824,
            ["source:HEROKU_POSTGRESQL_TEAL", "default:tag"],
          ],
          [
          ...
          [
            "heroku.postgres.active-connections",
            4,
            ["source:HEROKU_POSTGRESQL_TEAL", "default:tag"],
          ],
          [
            "heroku.postgres.waiting-connections",
            0,
            ["source:HEROKU_POSTGRESQL_TEAL", "default:tag"],
          ],
          [
            "heroku.postgres.index-cache-hit-rate",
            0.99971,
            ["source:HEROKU_POSTGRESQL_TEAL", "default:tag"],
          ],
          [
            "heroku.postgres.table-cache-hit-rate",
            0.99892,
            ["source:HEROKU_POSTGRESQL_TEAL", "default:tag"],
          ],
          [
            "heroku.postgres.load-avg-1m",
            0.315,
            ["source:HEROKU_POSTGRESQL_TEAL", "default:tag"],
          ],
          [
            "heroku.postgres.load-avg-5m",
            0.22,
            ["source:HEROKU_POSTGRESQL_TEAL", "default:tag"],
          ],
          [
            "heroku.postgres.load-avg-15m",
            0.225,
            ["source:HEROKU_POSTGRESQL_TEAL", "default:tag"],
          ],
          [
            "heroku.postgres.read-iops",
            25.996,
            ["source:HEROKU_POSTGRESQL_TEAL", "default:tag"],
          ],
          [
            "heroku.postgres.write-iops",
            1.629,
            ["source:HEROKU_POSTGRESQL_TEAL", "default:tag"],
          ],
          [
            "heroku.postgres.memory-total",
            15666128,
            ["source:HEROKU_POSTGRESQL_TEAL", "default:tag"],
          ],
          [
            "heroku.postgres.memory-free",
            233092,
            ["source:HEROKU_POSTGRESQL_TEAL", "default:tag"],
          ],
          [
            "heroku.postgres.memory-cached",
            14836812,
            ["source:HEROKU_POSTGRESQL_TEAL", "default:tag"],
          ],
          [
            "heroku.postgres.memory-postgres",
            170376,
            ["source:HEROKU_POSTGRESQL_TEAL", "default:tag"],
          ],
        ]);
    ...
    return request(app)
      ...
      .then(function () {
        ...
        expect(StatsD.prototype.gauge.args).to.deep.equal([
          ["heroku.dyno.mailer", 1, ["default:tag"]],
          ["heroku.dyno.web", 3, ["default:tag"]],
        ]);
